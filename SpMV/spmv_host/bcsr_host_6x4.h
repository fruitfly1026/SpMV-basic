...
void spmv_bcsr_serial_host_6x4(const bcsr_matrix<IndexType, ValueType>& bcsr, 
                          const ValueType * x,  
                          ValueType * y)
{
...
//  {
    for (I=0; I<num_block_rows; I++, yp += 6)
    {
      ...
      for (K=bcsr.bAp[I]; K<bcsr.bAp[I+1]; K++, bAj_tmp ++, bAx_tmp += 6*4 )
      {
        j0 = bAj_tmp[0];
        const ValueType *xp = x + j0;
        register ValueType _x0 = xp[0], _x1 = xp[1], _x2 = xp[2], _x3 = xp[3];
        _y0 += bAx_tmp[0] * _x0;
        _y1 += bAx_tmp[4] * _x0;
        _y2 += bAx_tmp[8] * _x0;
        _y3 += bAx_tmp[12] * _x0;
