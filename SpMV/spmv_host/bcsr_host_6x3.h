...
void spmv_bcsr_serial_host_6x3(const bcsr_matrix<IndexType, ValueType>& bcsr, 
                          const ValueType * x,  
                          ValueType * y)
{
...
//  {
    for (I=0; I<num_block_rows; I++, yp += 6)
    {
      ...
      for (K=bcsr.bAp[I]; K<bcsr.bAp[I+1]; K++, bAj_tmp ++, bAx_tmp += 6*3 )
      {
        j0 = bAj_tmp[0];
        const ValueType *xp = x + j0;
        register ValueType _x0 = xp[0], _x1 = xp[1], _x2 = xp[2];
        _y0 += bAx_tmp[0] * _x0;
        _y1 += bAx_tmp[3] * _x0;
        _y2 += bAx_tmp[6] * _x0;
        _y3 += bAx_tmp[9] * _x0;
