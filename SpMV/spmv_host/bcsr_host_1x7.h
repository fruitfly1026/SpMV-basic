...
void spmv_bcsr_serial_host_1x7(const bcsr_matrix<IndexType, ValueType>& bcsr, 
                          const ValueType * x,  
                          ValueType * y)
{
...
//  {
    for (I=0; I<num_block_rows; I++, yp += 1)
    {
      register ValueType _y0 = 0;
      for (K=bcsr.bAp[I]; K<bcsr.bAp[I+1]; K++, bAj_tmp ++, bAx_tmp += 1*7 )
      {
        j0 = bAj_tmp[0];
