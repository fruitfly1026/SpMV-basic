                                   ...
                                   const IndexType num_cols)
{
    ...
    IndexType * Bp = new_array<IndexType>(num_rows + 1);
    IndexType * Bj = new_array<IndexType>(num_nonzeros);
    ValueType * Bx = new_array<ValueType>(num_nonzeros);
    ...
    for(IndexType i = 0; i < num_rows; i++){ //Ap[num_rows] == Bp[num_rows] by construction
        if(Ap[i] != Bp[i]){
            delete_array(Bp); delete_array(Bj); delete_array(Bx);
            return false;
        }
    }

    //Compute C = A^T^T = A (C will have sorted indices)
    IndexType * Cp = new_array<IndexType>(num_rows + 1);
    IndexType * Cj = new_array<IndexType>(num_nonzeros);
    ValueType * Cx = new_array<ValueType>(num_nonzeros);
    ...
    for(IndexType i = 0; i < num_nonzeros; i++){
        if(Bj[i] != Cj[i]){         
            delete_array(Bp); delete_array(Bj); delete_array(Bx);
            delete_array(Cp); delete_array(Cj); delete_array(Cx);
            return false;
        }
    }
                        ...
                        bool Kway = false)
{
    ...
    int * int_partition = new_array<int>(partition.size()); //because METIS expects an int array
    ...
    if(is_symmetric){
        ...
        coo_AtA.I = new_array<IndexType>(2*graph.num_nonzeros);
        coo_AtA.J = new_array<IndexType>(2*graph.num_nonzeros);
        coo_AtA.V = new_array<ValueType>(2*graph.num_nonzeros);
        ...
        delete_array(coo_AtA.I);  delete_array(coo_AtA.J);  delete_array(coo_AtA.V);
        delete_array(csr_AtA.Ap); delete_array(csr_AtA.Aj); delete_array(csr_AtA.Ax);
    }
